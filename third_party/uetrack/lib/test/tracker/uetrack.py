...
class UETrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(UETrack, self).__init__(params)
        network = build_uetrack_inference(params.cfg)
        ...
        if DATASET_NAME in 'TNL2K':
            self.task_index_batch =  torch.tensor([1]).cuda()
        elif DATASET_NAME in 'DEPTHTRACK':
            self.task_index_batch =  torch.tensor([2]).cuda()
        elif DATASET_NAME in 'LASHER':
            self.task_index_batch =  torch.tensor([3]).cuda()
        elif DATASET_NAME in 'VISEVENT':
            self.task_index_batch =  torch.tensor([4]).cuda()
        elif DATASET_NAME in ['LASOT','GOT10K','TRACKINGNET','NFS','UAV']:
            self.task_index_batch =  torch.tensor([0]).cuda()
        else:
    ...
    def initialize(self, image, info: dict):
        ...
        if self.multi_modal_language:
            ...
            with torch.no_grad():
                self.text_src = self.network.forward_textencoder_inference(text_data=text_data)
        else:
            self.text_src = None
    ...
    def track(self, image, info: dict = None):
        ...
        # run the encoder
        with torch.no_grad():
            enc_opt,feature_list,_ = self.network.forward_encoder(self.template_list,
                                                   search_list,
        ...
        return {"target_bbox": self.state,
                "confidence": float(conf_score.detach().mean().cpu())}
