...
class ODFirstGraph(nn.Module):
    """First tracking step: upstream ODTrack has one template and no query."""
    def __init__(self, network):
        super().__init__()
        self.backbone = network.backbone
        self.box_head = network.box_head
    ...
    def forward(self, template0, search):
        x, _aux = self.backbone(z=[template0], x=search,
...
class ODStateGraph(ODFirstGraph):
    """Steady-state step with one template plus the causal track query."""
    def forward(self, template0, search, track_query):
        x, _aux = self.backbone(z=[template0], x=search,
                                ce_template_mask=None, ce_keep_rate=None,
                                return_last_attn=False, track_query=track_query,
                                token_len=self.token_len)
        feat_last = x[-1] if isinstance(x, (list, tuple)) else x
        enc_opt = feat_last[:, -self.feat_len_s:]
        new_query = x[:, :self.token_len]
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--mode", choices=["three_query", "first", "state"], default="three_query",
                    help="three_query is the legacy repeated-template graph; first/state mimic the upstream first step and query state")
    ...
    graph = {"three_query": ODFrameGraph, "first": ODFirstGraph,
             "state": ODStateGraph}[args.mode](network).eval()
    ...
    if args.mode == "three_query":
        ...
        tensors = (torch.zeros(template_shape), torch.zeros(search_shape))
        input_names = ["template0", "search"]
    else:
        tensors = (torch.zeros(template_shape), torch.zeros(search_shape), torch.zeros(query_shape))
        input_names = ["template0", "search", "track_query"]
